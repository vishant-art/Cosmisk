...
Storage (gitignored, flat JSON to match the standalone style):
  history/<account>__<level>.json   { "months": { "YYYY-MM": {facts...} } }
...
def _path(account: str, level: str) -> Path:
    return HISTORY_DIR / f"{account}__{level}.json"


def load(account: str, level: str) -> dict:
    p = _path(account, level)
    if not p.is_file():
        return {}
    try:
        return json.loads(p.read_text(encoding="utf-8")).get("months", {})
    except (json.JSONDecodeError, OSError):
        return {}


def save(account: str, level: str, months: dict) -> None:
    HISTORY_DIR.mkdir(exist_ok=True)
    _path(account, level).write_text(
        json.dumps({"months": months}, indent=2, ensure_ascii=False), encoding="utf-8")


# --- build ------------------------------------------------------------------

def ensure(account: str, level: str, facts_for_month, today: date,
           months_back: int = RETENTION_MONTHS, progress=None) -> dict:
