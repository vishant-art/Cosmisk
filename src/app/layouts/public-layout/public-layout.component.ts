...
@Component({
  ...
  template: `
    <!-- Fixed Header -->
    <header class="fixed top-0 left-0 right-0 z-50 h-[72px] bg-white/80 backdrop-blur-md border-b border-divider">
      <div class="max-w-7xl mx-auto h-full px-6 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-8">
          <a routerLink="/" class="text-sm font-body font-medium text-navy hover:text-accent no-underline transition-colors">Product</a>
          <a routerLink="/pricing" class="text-sm font-body font-medium text-navy hover:text-accent no-underline transition-colors">Pricing</a>
          <a class="text-sm font-body font-medium text-navy hover:text-accent no-underline transition-colors cursor-pointer">
            For Agencies
            <span class="ml-1 px-1.5 py-0.5 bg-accent/10 text-accent text-[10px] font-bold rounded">NEW</span>
          </a>
        </nav>

        <!-- Auth buttons -->
        <div class="flex items-center gap-3">
          <a routerLink="/login" class="btn-ghost !text-sm no-underline hidden sm:inline-flex">Log In</a>
