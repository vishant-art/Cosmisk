/**
 * Dashboard Routes Tests (Postgres integration)
 *
 * Tests /dashboard/kpis, /dashboard/chart, and /dashboard/insights endpoints
 * against the migrated Neon TEST BRANCH via the pg integration harness
 * (DB_BACKEND=postgres). External services (Meta API, token-crypto) are mocked.
 */
...
async function seedBaseUser() {
  ...
  await getDbAdapter().run(
    ...
    [testUserId, 'Dashboard Test', 'dashboard@test.com', hash, 'user', 'growth'],
  );
...
describe('GET /dashboard/chart', () => {
  ...
  it('returns chart data when Meta token exists', async () => {
    // Seed a meta token for the test user (FK: user already seeded).
    await getDbAdapter().run(
...
describe('GET /dashboard/insights', () => {
  ...
  it('returns meta_connected false when no Meta token for a new user', async () => {
    ...
    await getDbAdapter().run(
      'INSERT INTO users (id, name, email, password_hash, role, plan) VALUES (?, ?, ?, ?, ?, ?)',
      [freshUserId, 'Fresh User', 'fresh@test.com', hash, 'user', 'free'],
    );
  ...
  it('returns insights array when Meta token exists', async () => {
    // Seed a meta token for the test user so insights resolves a connection.
    // (pg harness resets between tests, so seed it here explicitly.)
    await getDbAdapter().run(
      'INSERT INTO meta_tokens (user_id, encrypted_access_token, meta_user_id, meta_user_name) VALUES (?, ?, ?, ?)',
      [testUserId, 'encrypted_mock_token', 'meta_u1', 'Meta User'],
    );
