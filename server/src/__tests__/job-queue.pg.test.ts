/**
 * Tests for job-queue.ts — concurrent job processor.
 *
 ...
 * via pg.reset() (TRUNCATE). FK seed order: users -> creative_sprints ->
 * creative_jobs -> cost_ledger.
 */
...
async function insertTestSprint(overrides: { id?: string; status?: string } = {}) {
  const sprintId = overrides.id || uuidv4();
  await getDbAdapter().run(
    'INSERT INTO creative_sprints (id, user_id, name, status, total_creatives) VALUES (?, ?, ?, ?, ?)',
    [sprintId, TEST_USER_ID, 'Test Sprint', overrides.status || 'generating', 5],
  );
  return sprintId;
}

async function insertTestJob(sprintId: string, overrides: {
  id?: string;
  status?: string;
  priority?: number;
  retryCount?: number;
...
describe('Job Queue', () => {
  ...
  describe('recoverInterruptedSprints', () => {
    it('resets generating/polling jobs to pending', async () => {
      const sprintId = await insertTestSprint();
      const generatingJob = await insertTestJob(sprintId, { status: 'generating' });
      const pollingJob = await insertTestJob(sprintId, { status: 'polling' });
      const completedJob = await insertTestJob(sprintId, { status: 'completed' });
      ...
      const genRow = await getDbAdapter().get('SELECT status, retry_count FROM creative_jobs WHERE id = ?', [generatingJob]) as any;
      expect(genRow.status).toBe('pending');
      expect(genRow.retry_count).toBe(1); // incremented

      const pollRow = await getDbAdapter().get('SELECT status FROM creative_jobs WHERE id = ?', [pollingJob]) as any;
      expect(pollRow.status).toBe('pending');

      const compRow = await getDbAdapter().get('SELECT status FROM creative_jobs WHERE id = ?', [completedJob]) as any;
      expect(compRow.status).toBe('completed'); // untouched
    });

    it('moves sprint with all completed jobs to reviewing', async () => {
      const sprintId = await insertTestSprint({ status: 'generating' });
      await insertTestJob(sprintId, { status: 'completed' });
      await insertTestJob(sprintId, { status: 'completed' });
      await insertTestJob(sprintId, { status: 'failed' });

      const { recoverInterruptedSprints } = await import('../services/job-queue.js');
      await recoverInterruptedSprints();

      const sprint = await getDbAdapter().get('SELECT status FROM creative_sprints WHERE id = ?', [sprintId]) as any;
      expect(sprint.status).toBe('reviewing');
    });

    it('handles no interrupted sprints gracefully', async () => {
      const sprintId = await insertTestSprint({ status: 'completed' });
      await insertTestJob(sprintId, { status: 'completed' });
  ...
  describe('Job status transitions (schema-level)', () => {
    it('jobs start with pending status and retry_count 0', async () => {
      const sprintId = await insertTestSprint();
      const jobId = await insertTestJob(sprintId);

      const job = await getDbAdapter().get('SELECT status, retry_count FROM creative_jobs WHERE id = ?', [jobId]) as any;
      expect(job.status).toBe('pending');
      expect(job.retry_count).toBe(0);
    });

    it('tracks retry count increment correctly', async () => {
      const sprintId = await insertTestSprint();
      const jobId = await insertTestJob(sprintId, { retryCount: 0 });
      ...
      const job = await getDbAdapter().get('SELECT status, retry_count, error_message FROM creative_jobs WHERE id = ?', [jobId]) as any;
      expect(job.status).toBe('pending');
    ...
    it('marks job as failed when max retries exceeded', async () => {
      const sprintId = await insertTestSprint();
      const jobId = await insertTestJob(sprintId, { retryCount: 2 }); // MAX_RETRIES is 2
      ...
      const job = await getDbAdapter().get('SELECT status, error_message, completed_at FROM creative_jobs WHERE id = ?', [jobId]) as any;
  ...
  describe('Sprint progress tracking (schema-level)', () => {
    it('updates sprint completed/failed counts from job stats', async () => {
      const sprintId = await insertTestSprint();

      // Insert jobs with various statuses
      await insertTestJob(sprintId, { status: 'completed', costCents: 100 });
      await insertTestJob(sprintId, { status: 'completed', costCents: 150 });
      await insertTestJob(sprintId, { status: 'failed' });
      await insertTestJob(sprintId, { status: 'pending' });

      // Replicate updateSprintProgress logic
      const stats = await getDbAdapter().get(`
        SELECT
      ...
      const sprint = await getDbAdapter().get('SELECT * FROM creative_sprints WHERE id = ?', [sprintId]) as any;
  ...
  describe('Cost ledger', () => {
    it('records cost entries for completed jobs', async () => {
      const sprintId = await insertTestSprint();
      const jobId = await insertTestJob(sprintId, { format: 'video', costCents: 200 });
      ...
      const ledger = await getDbAdapter().get('SELECT * FROM cost_ledger WHERE job_id = ?', [jobId]) as any;
    ...
    it('does NOT record cost entries when cost is 0', async () => {
      const sprintId = await insertTestSprint();
      const jobId = await insertTestJob(sprintId, { costCents: 0 });
      ...
      if (costCents > 0) {
        await getDbAdapter().run(`
          INSERT INTO cost_ledger (user_id, sprint_id, job_id, api_provider, operation, cost_cents, metadata)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [TEST_USER_ID, sprintId, jobId, 'mock-provider', 'video_gen', costCents, '{}']);
      }

      const count = (await getDbAdapter().get('SELECT COUNT(*) as c FROM cost_ledger WHERE job_id = ?', [jobId]) as any).c;
      expect(count).toBe(0);
    });
  });
  ...
  describe('Concurrent processing limits', () => {
    it('counts active jobs correctly', async () => {
      ...
      await insertTestJob(sprintId, { status: 'generating' });
      await insertTestJob(sprintId, { status: 'generating' });
      await insertTestJob(sprintId, { status: 'polling' });
      await insertTestJob(sprintId, { status: 'completed' }); // should not count
      await insertTestJob(sprintId, { status: 'pending' }); // should not count

      const activeCount = (await getDbAdapter().get(
    ...
    it('fetches pending jobs ordered by priority DESC, created_at ASC', async () => {
      const sprintId = await insertTestSprint();

      const lowPriJob = await insertTestJob(sprintId, { status: 'pending', priority: 1 });
      const highPriJob = await insertTestJob(sprintId, { status: 'pending', priority: 10 });
      const medPriJob = await insertTestJob(sprintId, { status: 'pending', priority: 5 });

      const jobs = await getDbAdapter().all(
  ...
  describe('Job completion detection', () => {
    it('detects when all jobs are done (completed + failed)', async () => {
      const sprintId = await insertTestSprint();
      await insertTestJob(sprintId, { status: 'completed' });
      await insertTestJob(sprintId, { status: 'completed' });
      await insertTestJob(sprintId, { status: 'failed' });

      const remaining = (await getDbAdapter().get(
    ...
    it('detects remaining jobs when some are still pending', async () => {
      const sprintId = await insertTestSprint();
      await insertTestJob(sprintId, { status: 'completed' });
      await insertTestJob(sprintId, { status: 'pending' });
      await insertTestJob(sprintId, { status: 'generating' });
  ...
  describe('startSprintGeneration deduplication', () => {
    it('does not start duplicate processors for the same sprint', async () => {
      const { startSprintGeneration } = await import('../services/job-queue.js');
