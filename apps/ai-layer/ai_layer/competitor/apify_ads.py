...
def load_ads(key: str, brand_id: str | None = None) -> dict | None:
    intel = _repo.load_competitor_intel(key, brand_id=brand_id)
    return intel["ads"] if intel else None


def save_ads(key: str, record: dict, brand_id: str | None = None) -> None:
    _repo.save_competitor_ads(key, record, brand_id=brand_id)
