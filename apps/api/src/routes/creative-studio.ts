...
/**
 * Live generation progress, keyed by generationId. The ai-layer job emits stage
 * milestones ("Brand kit decided", "Ad 2/3 generated", "Video done"); the poller
 * mirrors them here so GET /generation/:id can surface them WITHOUT overloading the
 * machine-valued `status` (the frontend polls on status === 'generating'). In-memory,
 * cleared on terminal state — durable progress is part of the deferred DB phase.
 */
const genProgress = new Map<string, { stage: string; progress: string[] }>();
...
Return ONLY valid JSON, no markdown.`,
    ...
    try {
      ...
      return {
        ...
        generation: {
          ...
          stage: prog?.stage ?? null,        // live milestone while generating
  ...
  app.get('/asset/:jobId/:file', async (request, reply) => {
...
async function processGenerationViaAiLayer(
  ...
  const db = getDbAdapter();
  const now = () => new Date().toISOString();
  const setOutput = async (outputId: string, status: string, outputJson?: string, errorMessage?: string, costCents = 0) => {
    await db.run(
      'UPDATE studio_outputs SET status = ?, output_json = ?, error_message = ?, cost_cents = ?, updated_at = ? WHERE id = ?',
      [status, outputJson ?? null, errorMessage ?? null, costCents, now(), outputId],
    );
  };
  ...
  try {
    ...
    // Poll the Python job (generation takes minutes; cap at ~8 min), mirroring its
    // live stage milestones so the UI can show progress mid-generation.
    const deadline = Date.now() + 8 * 60_000;
    let job = await getCreativeJob(jobId);
    genProgress.set(generationId, { stage: job.stage, progress: job.progress });
    ...
    if (job.status !== 'complete') {
      ...
      await db.run('UPDATE studio_generations SET status = ?, updated_at = ? WHERE id = ?', ['failed', now(), generationId]);
      return;
    }

    // Rewrite ai-layer asset paths to the apps/api proxy so the browser only hits /api.
    const proxy = (u: string) => `/api/creative-studio/asset/${jobId}/${u.split('/').pop()}`;
    const images = job.assets.map(a => ({
      image_url: proxy(a.url),
      aspect_ratio: a.fmt,
      status: 'completed',
      headline: a.copy?.headline ?? null,
      concept: a.concept,
    }));
    const videoOut = job.video ? { video_url: proxy(job.video.url), status: 'completed' } : null;
    ...
    for (const f of formats) {
      ...
      if (f === 'video') {
        if (videoOut) await setOutput(oid, 'completed', JSON.stringify(videoOut), undefined, costCents);
        else await setOutput(oid, 'pending', JSON.stringify({ message: 'No video produced for this run.' }));
      } else {
        await setOutput(oid, 'completed', JSON.stringify(images), undefined, costCents);
      }
    }
    await db.run('UPDATE studio_generations SET status = ?, updated_at = ? WHERE id = ?', ['completed', now(), generationId]);
  } catch (err: any) {
    ...
    await db.run('UPDATE studio_generations SET status = ?, updated_at = ? WHERE id = ?', ['failed', new Date().toISOString(), generationId]).catch(() => { /* best-effort */ });
  } finally {
    // Keep the final stage briefly for a last poll, then drop it (in-memory only).
    setTimeout(() => genProgress.delete(generationId), 60_000).unref?.();
  }
}
