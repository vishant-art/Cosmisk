...
| **`processGeneration()`** — legacy TS generation path (Flux + TS scorer) | **On disk, never called.** The `/generate` branch now always routes to `processGenerationViaAiLayer`; returns **503** if the ai-layer is unconfigured (no TS fallback). | `creative-studio.ts` (`~:411`) |
