...
**Created:** 2026-07-20 · **Owner:** ai-engineer (creative subtree, `dryayeet`) · **Status:** Phases 1-4 COMPLETE (both trees)
...
4. **Phase 4 — SHIPPED:** motion QA (#10): the temporal VLM critic now gets a consecutive-frame MOTION STRIP so it can see frozen_frame / morphing / identity_drift (the deterministic per-shot `check_shot_motion` already covered fully-frozen shots, which is why a redundant timeline-level check would only false-fail the held-frame test fixtures). Seed-the-person (#4b): a new `_hero_persona_seed` + an OPT-IN `hero_with_creator` flag (`VideoRenderRequest.hero_with_creator` + rnd `--hero-with-creator`, default OFF) that seeds the creator WITH the product on hero shots; kept off because the product-fidelity-vs-person-consistency tradeoff genuinely conflicts with the person-free `_product_seed` bug-fix and needs a paid live run to validate before defaulting on. Also synced the rnd tree with the earlier apps-only caption-crop fix.
