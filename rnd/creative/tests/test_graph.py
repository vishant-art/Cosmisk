...
The store seam is `library` (rnd's JSON stand-in for the deployed Neon repository); it is
monkeypatched, so nothing here touches disk.
"""
...
def test_a_winner_only_corpus_says_nothing():
...
def test_a_known_ad_is_never_torn_down_twice(monkeypatch):
    ...
    def save_teardown(brand_id, ad_id, cohort, template_json, thumb=None):
        seen[ad_id] = cohort
        return True

    monkeypatch.setattr(library, "has_teardown", has_teardown)
    monkeypatch.setattr(library, "save_teardown", save_teardown)
...
def test_no_database_never_breaks_a_run(monkeypatch):
    ...
    monkeypatch.setattr(library, "has_teardown", boom)
    monkeypatch.setattr(library, "save_teardown", boom)
