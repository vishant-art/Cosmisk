...
from schemas import ArmResult, AxisFinding, CreativePrior  # noqa: E402


def _repo():
    """The store seam. In rnd this is library.py (JSON files, no database); in the deployed
    tree it is ai_layer.db.repository (Neon). Same interface, so this module is otherwise
    identical in both."""
    import library
    return library
