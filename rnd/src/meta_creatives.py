...
def fetch_winning_creatives(token: str, account: str, *, preset: str = "last_30d",
                            ...
                            log=print) -> list[CreativeAsset]:
    """Winners only. DEPRECATED for any learning use: a corpus selected on the outcome
    cannot support an effect estimate (UGC-D5). Kept for conditioning-only callers
    that just want reference pixels. Prefer fetch_creative_cohort."""
    return fetch_creative_cohort(token, account, preset=preset, top_n=top_n, bottom_n=0,
                                 min_spend=0.0, out_dir=out_dir, want_video=want_video,
                                 log=log)
